...
import type {EditorTool} from '../state/ui.store'

import {
  AREA_COLORS,
  DEFAULT_AREA_STYLE,
} from '../constants/area-style'
import {DEFAULT_PERSON_RADIUS} from '../constants/person-defaults'
import {SHAPE_STROKE_COLOR} from '../constants/shape-style'
import {
  DEFAULT_WALL_COLOR,
  DEFAULT_WALL_THICKNESS,
} from '../constants/wall-style'
import {getEffectiveHorizontalFov} from '../services/camera-optics'
  ...
  intersections.features.forEach((feature) => {
    ...
    if (geometry.type === 'MultiPoint') {
      geometry.coordinates.forEach((coordinate) => {
        const coords = coordinate as unknown as GeoPoint
        const meters =
