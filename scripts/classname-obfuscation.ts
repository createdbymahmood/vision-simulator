...
const addClassTokensToSet = (value: string, classTokens: Set<string>) => {
  for (const classToken of tokenizeClassValue(value)) {
    classTokens.add(classToken)
  }
}

const collectClassTokensFromTemplateLiteral = (
  ...
  for (const templateElement of templateNode.quasis) {
    ...
    addClassTokensToSet(cookedValue, classTokens)
  }

  for (const expression of templateNode.expressions) {
    collectClassTokensFromExpression(expression, classTokens)
  }
}

const collectClassTokensFromExpression = (
  ...
  if (t.isStringLiteral(expression)) {
    addClassTokensToSet(expression.value, classTokens)
    return
  }

  if (t.isTemplateLiteral(expression)) {
    collectClassTokensFromTemplateLiteral(expression, classTokens)
    return
  }

  if (t.isConditionalExpression(expression)) {
    collectClassTokensFromExpression(expression.consequent, classTokens)
    collectClassTokensFromExpression(expression.alternate, classTokens)
    return
  }

  if (t.isLogicalExpression(expression)) {
    collectClassTokensFromExpression(expression.right, classTokens)

    if (expression.operator === '||' || expression.operator === '??') {
      collectClassTokensFromExpression(expression.left, classTokens)
    }
  ...
  if (t.isArrayExpression(expression)) {
    for (const element of expression.elements) {
      ...
      collectClassTokensFromExpression(element, classTokens)
    }
  ...
  if (t.isObjectExpression(expression)) {
    for (const property of expression.properties) {
      if (t.isSpreadElement(property)) {
        collectClassTokensFromExpression(property.argument, classTokens)
        continue
      }
      ...
      if (property.computed) {
        collectClassTokensFromExpression(property.key, classTokens)
      }

      if (t.isStringLiteral(property.key)) {
        addClassTokensToSet(property.key.value, classTokens)
      }
    }
  ...
  if (t.isParenthesizedExpression(expression)) {
    collectClassTokensFromExpression(expression.expression, classTokens)
    return
  }
  ...
  if (t.isCallExpression(expression) && isClassComposerCall(expression)) {
    if (isCvaCall(expression)) {
      collectClassTokensFromCvaCallExpression(expression, classTokens)
      return
    }

    for (const argument of expression.arguments) {
      if (t.isSpreadElement(argument)) {
        collectClassTokensFromExpression(argument.argument, classTokens)
        continue
      }

      collectClassTokensFromExpression(argument, classTokens)
    }
  }
}

const collectClassTokensFromCvaVariantsExpression = (
  expression: t.Node,
  classTokens: Set<string>,
) => {
  if (!t.isObjectExpression(expression)) {
  ...
  for (const variantGroupProperty of expression.properties) {
    ...
    for (const variantOptionProperty of variantGroupProperty.value.properties) {
      ...
      collectClassTokensFromExpression(variantOptionProperty.value, classTokens)
    }
  }
}

const collectClassTokensFromCvaCompoundVariantsExpression = (
  expression: t.Node,
  classTokens: Set<string>,
) => {
  if (!t.isArrayExpression(expression)) {
  ...
  for (const element of expression.elements) {
    ...
    for (const property of element.properties) {
      ...
      collectClassTokensFromExpression(property.value, classTokens)
    }
  }
}

const collectClassTokensFromCvaConfigExpression = (
  expression: t.Node,
  classTokens: Set<string>,
) => {
  if (!t.isObjectExpression(expression)) {
  ...
  for (const property of expression.properties) {
    ...
    if (keyName === 'variants') {
      collectClassTokensFromCvaVariantsExpression(property.value, classTokens)
      continue
    }

    if (keyName === 'compoundVariants') {
      collectClassTokensFromCvaCompoundVariantsExpression(
        property.value,
        classTokens,
      )
      continue
...
const collectClassTokensFromCvaCallExpression = (
  ...
  if (firstArgument && !t.isSpreadElement(firstArgument)) {
    collectClassTokensFromExpression(firstArgument, classTokens)
  }

  const secondArgument = expression.arguments[1]

  if (secondArgument && !t.isSpreadElement(secondArgument)) {
    collectClassTokensFromCvaConfigExpression(secondArgument, classTokens)
  }

  for (const trailingArgument of expression.arguments.slice(2)) {
    if (t.isSpreadElement(trailingArgument)) {
      collectClassTokensFromExpression(trailingArgument.argument, classTokens)
      continue
    }

    collectClassTokensFromExpression(trailingArgument, classTokens)
  }
}
...
const collectProjectSourceFilePaths = (directoryPath: string): string[] => {
  ...
  for (const directoryEntry of directoryEntries) {
    ...
    if (directoryEntry.isDirectory()) {
      sourceFilePaths.push(...collectProjectSourceFilePaths(entryPath))
      continue
    }
    ...
    if (!SUPPORTED_SOURCE_EXTENSIONS.has(path.extname(directoryEntry.name))) {
      continue
    }
    ...
    traverse(parsedAst, {
      CallExpression(path) {
        ...
        if (isCvaCall(path.node)) {
          collectClassTokensFromCvaCallExpression(path.node, classNames)
          return
        }

        for (const argument of path.node.arguments) {
          if (t.isSpreadElement(argument)) {
            collectClassTokensFromExpression(argument.argument, classNames)
            continue
          }
...
const createClassMap = (
  classNames: Iterable<string>,
  composerPreservedClassNames: ReadonlySet<string>,
) => {
  ...
  for (const className of [...classNames].sort((left, right) =>
    ...
    // Keep class-composer tokens readable so runtime twMerge/cva conflict
    // resolution remains correct when consumers pass overriding className values.
    if (
      isPreservedClassName(className) ||
      composerPreservedClassNames.has(className)
    ) {
      continue
    }
