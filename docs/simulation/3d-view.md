...
The 3D view renders a simulation world based on your editor scene. Walls, shapes, cameras, and people are shown as 3D meshes with lighting and shadows. The ground plane is textured with Mapbox tiles in Map Mode and a neutral grid in Canvas Mode, and the Map visibility toggle swaps tiles for a grid. Switching Map and Canvas uses a smooth texture fade.
...
Double-clicking a camera or person focuses the orbit target, and a smooth animation centers and zooms to the object.
